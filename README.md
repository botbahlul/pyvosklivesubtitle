...
Pay attension to those folders name, because their names should contain the language codes in __init__.py, esspecially for Chinese language, which in __init__.py its code is "zh", so you should rename extracted the downloaded model to "vosk-model-small-zh-0.22". This is needed for GoogleTranslate funcion.
